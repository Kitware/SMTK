...
#include "attribute/AttributeReader.h"
...
bool AttributeReader::read(Manager &manager, const std::string &filename)
{
  ...
  xml_node root = doc.child("SLCTK_AttributeManager");
  if (!root)
    {
    this->m_errorMessages = "Error: Can not find root node: SLCTK_AttributeManager\n";
    return true;
    }
  ...
  if (!xatt)
    {
    this->m_errorMessages = "Error: Can not find XML Attribute Version in node: SLCTK_AttributeManager\n";
    return true;
    }
