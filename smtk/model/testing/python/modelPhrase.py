...
class TestModelPhrases(unittest.TestCase):
    ...
    def testPhrases(self):
        ...
        self.assertEqual(self.elist.title(), '1 volumetric model',
                         'Unexpected top-level title.')
        ...
        self.assertEqual(s1[0].subtitle(), 'volumetric model',
                         'Unexpected model subtitle.')
        self.assertEqual(s1[0].relatedEntity(), self.models[0],
                         'Model phrase not related to model.')
        self.assertEqual(s1[0].relatedColor(), [
                         0.0, 0.0, 0.0, -1.0], 'Unexpected model color.')
        self.assertEqual(s1[0].isTitleMutable(), True,
        ...
        #    'Expected to find children where they were reported.')
        self.assertEqual([x.title() for x in s2], ['volume 0'],
                         'Unexpected grandchild titles.')

        self.spg.setDirectLimit(2)
        s3 = s2[0].subphrases()
        self.assertEqual([x.title() for x in s3], ['5 faces',
                                                   'pedigree id'], 'Expected a summary phrase.')
        self.assertTrue(s2[0].areSubphrasesBuilt(),
                        'Expected subphrases have been built.')

        # Test that marking a phrase dirty forces its children to be rebuilt
        s2[0].markDirty(True)
        self.assertFalse(s2[0].areSubphrasesBuilt(
        ), 'Expected subphrases need to be built after marking dirty.')
