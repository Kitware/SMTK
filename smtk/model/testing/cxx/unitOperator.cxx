...
int WillOperateWatcher(Operator::EventType event, const Operator& op, void* user)
{
  test(event == smtk::operation::Operator::WILL_OPERATE,
...
int DidOperateWatcher(
  Operator::EventType event, const Operator& op, Operator::Result result, void* user)
{
  test(event == smtk::operation::Operator::DID_OPERATE,
...
protected:
  Operator::Result operateInternal() override
  {
    return this->createResult(this->specification()->findInt("shouldSucceed")->value()
        ? TestOutcomeOperator::OPERATION_SUCCEEDED
        : TestOutcomeOperator::OPERATION_FAILED);
  }
};
...
void testExPostFactoOperatorRegistration(smtk::model::Manager::Ptr manager)
{
...
void testOperatorOutcomes(smtk::model::Manager::Ptr manager)
{
  ...
  op->observe(smtk::operation::Operator::WILL_OPERATE, WillOperateWatcher, &shouldCancel);
  op->observe(smtk::operation::Operator::DID_OPERATE, DidOperateWatcher, &numberOfFailedOperations);

  // Verify that ableToOperate() is called and complains:
  test(op->operate()->findInt("outcome")->value() == smtk::operation::Operator::UNABLE_TO_OPERATE,
  ...
  op->m_able = true;
  test(op->operate()->findInt("outcome")->value() == smtk::operation::Operator::OPERATION_CANCELED,
    "Operation should have been canceled.");
  std::cout << "--\n";

  // Verify that the operation fails when "shouldSucceed" parameter is zero.
  shouldCancel = 0;
  test(op->operate()->findInt("outcome")->value() == smtk::operation::Operator::OPERATION_FAILED,
    "Operation should have failed.");
  std::cout << "--\n";

  // Verify that removing observer bypasses cancellation.
  op->unobserve(smtk::operation::Operator::WILL_OPERATE, WillOperateWatcher, &shouldCancel);
  ...
  test(op->operate()->findInt("outcome")->value() == smtk::operation::Operator::OPERATION_SUCCEEDED,
  ...
  op->unobserve(
    smtk::operation::Operator::DID_OPERATE, DidOperateWatcher, &numberOfFailedOperations);
}
