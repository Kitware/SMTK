...
/**\brief Update the phrase with a new mesh collection.
  ...
  */
void MeshPhrase::updateMesh(const smtk::mesh::CollectionPtr& meshcollection)
{
  this->m_relatedCollection = meshcollection;
  ...
  if (this->areSubphrasesBuilt())
  {
    ...
    for (smtk::model::DescriptivePhrases::iterator it = meshsubs.begin(); it != meshsubs.end();
         ++it)
    {
...
/**\brief Update the phrase with a new meshset.
  * NOTE: This is not updating subphrases, just the related mesh
  */
void MeshPhrase::updateMesh(const smtk::mesh::MeshSet& meshset)
{
...
std::string MeshPhrase::title()
{
  ...
  if (this->isCollection())
  {
    //strText = "Meshes";
    strText =
      this->m_relatedCollection->name().empty() ? "Meshes" : this->m_relatedCollection->name();
  }
