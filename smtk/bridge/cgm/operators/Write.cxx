...
smtk::model::OperatorResult Write::operateInternal()
{
  ...
  smtk::model::Models bodies = this->specification()->associatedModelEntities<smtk::model::Models>();
  ...
  for (it = bodies.begin(); it != bodies.end(); ++it)
    ...
    if (refEntity)
      ...
      {
      smtkWarningMacro(log(),
        "No CGM entity for SMTK model \""
  ...
  if (s != CUBIT_SUCCESS)
    {
    smtkWarningMacro(this->manager()->log(),
      "Failed to save CGM model, status " << s);
