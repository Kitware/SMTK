...
void vtkGDALRasterReader::vtkGDALRasterReaderInternal::GenericReadData()
{
  ...
  {
    dir[0] = -1;
    geoSpacing[0] = -geoSpacing[0];
    at[0] = 4;
  }
  if(geoSpacing[1] < 0)
  {
    dir[1] = -1;
    geoSpacing[1] = -geoSpacing[1];
    at[1] = 5;
  }
  ...
  this->ImageData->SetOrigin(d[at[0]], d[at[1]], 0);

  this->Convert<VTK_TYPE, RAW_TYPE>(rawImageData, dir, destWidth, destHeight);
}
...
//-----------------------------------------------------------------------------
template <typename VTK_TYPE, typename RAW_TYPE>
void vtkGDALRasterReader
::vtkGDALRasterReaderInternal::Convert( std::vector<RAW_TYPE>& rawImageData,
                                       int direction[2],
                                       int targetWidth, int targetHeight)
{
  if (!this->ImageData)
  ...
  for (int j = 0, k = (direction[1]==1)?0:(targetHeight - 1); j < targetHeight; ++j, k += direction[1])
    {
    for (int i = 0, l = (direction[0]==1)?0:(targetWidth-1); i < targetWidth; ++i, l+=direction[0])
      {
