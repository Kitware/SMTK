...
public:
  ...
  ModelDomains() { this->UseDomainTag = false; }
  ...
  bool haveRegionId(vtkIdType regionId) const
  {
    ...
    for (const_iterator i = this->DomainsToRegions.begin();
         i != this->DomainsToRegions.end() && !haveId; ++i)
    {
  ...
  bool GetUseDomainTag() { return this->UseDomainTag; }
...
void vtkCMBModelBuilder::Operate(vtkDiscreteModelWrapper* modelWrapper, vtkAlgorithm* inputPoly)
{
  ...
  inputPoly->Update();
  vtkPolyData* modelPoly = vtkPolyData::SafeDownCast(inputPoly->GetOutputDataObject(0));
  ...
  if (modelRegionDomainMap)
  {
    ...
    modelPoly->GetFieldData()->RemoveArray(ModelParserHelper::GetModelPredefinedDomainSets());
  }
  ...
  {
    vtkErrorMacro(
      "Output from the input poly algorithm does not have modelFaceRegionsMap field data.");
    return;
  }
  ...
  for (vtkIdType i = 0; i < modelFaceIds->GetNumberOfTuples(); i++)
  {
    ...
    std::map<vtkIdType, vtkSmartPointer<vtkIdList> >::iterator it = modelFaceCells.find(faceId);
  ...
  model->SetMesh(mesh);
  vtkIdTypeArray* pointMapArray =
    vtkIdTypeArray::SafeDownCast(modelPoly->GetPointData()->GetArray("vtkOriginalPointIds"));
  vtkIdTypeArray* cellMapArray =
    vtkIdTypeArray::SafeDownCast(modelPoly->GetCellData()->GetArray("OrigCellIds"));
  ...
  for (md_iterator it = domains.begin(); it != domains.end(); it++, counter++)
  {
    ...
    for (regionIterator = it->second.begin(); regionIterator != it->second.end(); ++regionIterator)
    {
      ...
      vtkDiscreteModelRegion* region =
        vtkDiscreteModelRegion::SafeDownCast(model->BuildModelRegion(numFaces,
          &(RegionInfo[regionId].first[0]), &(RegionInfo[regionId].second[0]), regionId, material));
  ...
  if (modelPoly->GetPointData()->GetNumberOfArrays() != 0)
  {
    ...
    {
      vtkWarningMacro("Point data array name is " << modelPoly->GetPointData()->GetArrayName(ii));
    }
  }
  if (modelPoly->GetCellData()->GetNumberOfArrays() != 0)
  {
    ...
    {
      vtkWarningMacro("Cell data array name is " << modelPoly->GetCellData()->GetArrayName(ii));
    }
  }
  if (modelPoly->GetFieldData()->GetNumberOfArrays() != 0)
  {
    ...
    {
      vtkWarningMacro("Field data array name is " << modelPoly->GetFieldData()->GetArrayName(ii));
    }
  }
#endif

  vtkIdTypeArray* predefinedBoundarySet = vtkIdTypeArray::SafeDownCast(
    modelPoly->GetFieldData()->GetArray(ModelParserHelper::GetModelPredefinedBoundarySets()));
  if (predefinedBoundarySet)
  {
    ...
    faceIterator->Begin();
    for (vtkIdType faceIndex = 0;
         faceIndex < predefinedBoundarySet->GetNumberOfTuples() && !faceIterator->IsAtEnd();
         faceIndex++, faceIterator->Next())
    {
      vtkDiscreteModelEntity* entity =
        vtkDiscreteModelFace::SafeDownCast(faceIterator->GetCurrentItem());

      const vtkIdType boundaryCondition = predefinedBoundarySet->GetValue(faceIndex);
      boundaryMapping[boundaryCondition].push_back(entity);
    }
    faceIterator->Delete();
    //remove the mapping
    boundaryMapping.erase(-1);
    for (BCMap::iterator mit = boundaryMapping.begin(); mit != boundaryMapping.end(); mit++)
    {
    ...
    modelPoly->GetFieldData()->RemoveArray(ModelParserHelper::GetModelPredefinedBoundarySets());
  }
...
void vtkCMBModelBuilder::ProcessAs2DMesh(
  vtkDiscreteModelWrapper* modelWrapper, vtkPolyData* modelPolyData)
{
  vtkDiscreteModel* model = modelWrapper->GetModel();
  model->Reset();
  ...
  vtkIntArray* regionArray =
    vtkIntArray::SafeDownCast(modelPolyData->GetCellData()->GetArray("Region"));
  // keep mapping between new region and old region if it exists.
  std::map<vtkIdType, vtkIdType> newToOldRegionMap;
  vtkIntArray* oldRegionArray =
    vtkIntArray::SafeDownCast(modelPolyData->GetCellData()->GetArray("Original Region"));
  ...
  for (int i = 0; i < edgeNames->GetNumberOfValues(); i++)
  {
    ...
    vtkModelEdge* modelEdge = model->BuildModelEdge(
      modelVertices[edgeArray->GetValue(0)], modelVertices[edgeArray->GetValue(1)]);
  ...
  vtkStringArray* loopNames =
    vtkStringArray::SafeDownCast(modelPolyData->GetFieldData()->GetAbstractArray("Loop Names"));
       ...
       mainLoopIndex = innerLoopIndex)
  {
    vtkIdTypeArray* loopArray = vtkIdTypeArray::SafeDownCast(
      modelPolyData->GetFieldData()->GetArray(loopNames->GetValue(mainLoopIndex).c_str()));
    ...
    if (oldRegionArray && newToOldRegionMap.size() > 0)
    {
      ...
      newMaterial =
        vtkModelMaterial::SafeDownCast(model->GetModelEntity(vtkModelMaterialType, oldRegionId));
      if (!newMaterial)
      {
    ...
    delete[] edges;
    delete[] edgesDir;
    modelPolyData->GetFieldData()->RemoveArray(loopNames->GetValue(mainLoopIndex).c_str());
    ...
    for (innerLoopIndex = mainLoopIndex + 1; innerLoopIndex < loopNames->GetNumberOfValues();
         innerLoopIndex++)
    {
...
void vtkCMBModelBuilder::ComputePointInsideForRegion(
  vtkDiscreteModelRegion* region, vtkCellLocator* locator)
{
  double ptInside[3];

  vtkSmartPointer<vtkIdTypeArray> ptIdsArray = vtkSmartPointer<vtkIdTypeArray>::New();
  ...
  for (faces->Begin(); !faces->IsAtEnd(); faces->Next())
  {
    ...
    for (polys->InitTraversal(); polys->GetNextCell(numPts, pts); cellIndex++)
    {
      ...
      if (vtkCMBModelBuilder::IsPolygonConcave(faceGeometry->GetPoints(), numPts, pts))
      {
  ...
  vtkErrorMacro("Failed to set point inside for region " << region->GetUniquePersistentId());
  faces->Delete();
}
