...
struct WalkableEdge
  {
    WalkableEdge(const vtkIdType& _ptId1,const vtkIdType& _ptId2,const vtkIdType& _edgeId, const int& _dir) : ptId1(_ptId1), ptId2(_ptId2), edgeId(_edgeId), dir(_dir){}
    WalkableEdge(const vtkIdType& _ptId1,const vtkIdType& _ptId2,const vtkIdType& _edgeId) : ptId1(_ptId1), ptId2(_ptId2), edgeId(_edgeId), dir(-1){}
    WalkableEdge(const vtkIdType& _ptId1,const vtkIdType& _ptId2) : ptId1(_ptId1), ptId2(_ptId2), edgeId(-1), dir(-1){}
    ...
    bool operator <(const WalkableEdge &e) const
      ...
      if(e.edgeId == -1  || this->edgeId == -1 )
        {
        return  ((this->ptId1 < e.ptId1) ||
          (this->ptId1 == e.ptId1 && this->ptId2 < e.ptId2));
        }
...
class WalkableLoop
  ...
  public:
    ...
    WalkableLoop() : numEdges(-1), edge_arr(NULL), edge_dir_arr(NULL), edge_dir_flipped(false){}
    ~WalkableLoop()
      {
    ...
    void addEdge(const vtkIdType& ptId1, const vtkIdType& ptId2, const vtkIdType& arcId, vtkModelEdge* modelEdge)
      {
    ...
    void findEdge(const vtkIdType& ptId1, const vtkIdType& ptId2, std::list< WalkableEdge >& foundArcsAndDirs) const
      {
      std::set<WalkableEdge>::const_iterator order1Iter = order1.lower_bound(WalkableEdge(ptId1,ptId2));
      std::set<WalkableEdge>::const_iterator order2Iter = order2.lower_bound(WalkableEdge(ptId1,ptId2));
      for(;order1Iter != order1.end(); order1Iter++)
        {
        // Make sure we are still within bounds of the arcs we want to find
        if(ptId1 != order1Iter->ptId1 || (ptId2 != -1 && ptId2 != order1Iter->ptId2)) break;
        foundArcsAndDirs.push_back( WalkableEdge(order1Iter->ptId1,order1Iter->ptId2, order1Iter->edgeId,1) ); //return the arcId and clockwise direction
        }
      for(;order2Iter != order2.end(); order2Iter++)
        {
        // Make sure we are still within bounds of the arcs we want to find
        if(ptId1 != order2Iter->ptId1 || (ptId2 != -1 && ptId2 != order2Iter->ptId2)) break;
        foundArcsAndDirs.push_back( WalkableEdge(order2Iter->ptId1,order2Iter->ptId2, order2Iter->edgeId,0) ); //return the arcId and counterclockwise direction
        }
      }
    ...
    bool getLoopOrdering(int& _numEdges, vtkModelEdge**& _edge_arr, int*& _edge_dir_arr, bool flip_edge_dir=false)
      {
      ...
      vtkIdType targetEndPoint = (*curr).ptId1; //The last segment to complete this loop should have this id
      ...
      edge_dir_arr[curr_insert_pos] = 1; //because we grab the first segment from order 1 the dir is always clockwise
      curr_insert_pos++;

      this->removeEdge((*curr).ptId1,(*curr).ptId2,(*curr).edgeId); //Remove the edge so we don't have to check it again
      ...
      while( order1.size() > 0 )
        ...
        //There is a non-manifold situation attempt to resolve
          ...
          for(;nonmanifoldIter != foundEdges.end(); nonmanifoldIter++)
            ...
            if((*nonmanifoldIter).ptId1 == (*nonmanifoldIter).ptId2 && edge_dir_arr[curr_insert_pos-1] == (*nonmanifoldIter).dir)
              {
    ...
    vtkDiscreteModelWrapper* ModelWrapper, vtkAlgorithm* inputPolyAlg)
{
  ...
  vtkPolyData* input = vtkPolyData::SafeDownCast(
    inputPolyAlg->GetOutputDataObject(0));
  if(!input)
    {
  ...
  if(input->GetNumberOfCells() == 0 || input->GetNumberOfPoints() == 0)
    {
    vtkErrorMacro("Unable to set the model to be an empty data set. This is most likely caused by not having the relevant meshing support enabled");
    return;
    }
  ...
  vtkIdTypeArray* elementIds = vtkIdTypeArray::SafeDownCast(input->GetCellData()->GetArray("ElementIds"));
  if(!elementIds)
    {
  ...
  std::map<vtkIdType, vtkModelVertex* > nodeId2ModelVertex; // Needed to quickly build edges from just node ids
  ...
  for(int cellId = 0; cellId < input->GetNumberOfCells(); cellId++)
    ...
    if(cell->GetCellType() == VTK_VERTEX)
      ...
      vtkDiscreteModelVertex* cmbVertex = vtkDiscreteModelVertex::SafeDownCast(
        model->BuildModelVertex(cell->GetPointId(0)));
      if(cmbVertex)
        {
    ...
    if(cell->GetCellType() == VTK_LINE)
      ...
      std::map<vtkIdType, vtkSmartPointer<vtkIdList> >::iterator foundArc = arcId2CellIds.find(elementId);
      if(foundArc == arcId2CellIds.end())
        {
        foundArc = arcId2CellIds.insert(std::make_pair(elementId,vtkSmartPointer<vtkIdList>::New())).first;
        }
    ...
    if(cell->GetCellType() == VTK_TRIANGLE)
      ...
      std::map<vtkIdType, vtkSmartPointer<vtkIdList> >::iterator foundPoly = polyId2CellIds.find(elementId);
      if(foundPoly == polyId2CellIds.end())
        {
        foundPoly = polyId2CellIds.insert(std::make_pair(elementId,vtkSmartPointer<vtkIdList>::New())).first;
        }
  ...
  for(int arcIndex = 0; arcIndex < fieldArcIds->GetNumberOfTuples(); arcIndex++)
    ...
    vtkModelEdge* edge = model->BuildModelEdge(nodeId2ModelVertex[endpoint1],nodeId2ModelVertex[endpoint2]);
  ...
  std::map< vtkIdType, std::pair< std::list<vtkIdType>, vtkModelFace* > > polyId2InnerLoopsAndFaces; // on the first pass we can only create the outer loops
  ...
  for(int loopIndex = 0; loopIndex < fieldLoopInfo->GetNumberOfTuples(); loopIndex++)
    ...
    if(innerPoly > 0) // -1 and 0 are invalid ids
      ...
      std::map< vtkIdType, std::pair< std::list<vtkIdType>, vtkModelFace* > >::iterator foundPoly = polyId2InnerLoopsAndFaces.find(innerPoly);
      if( foundPoly == polyId2InnerLoopsAndFaces.end() )
        {
        foundPoly = polyId2InnerLoopsAndFaces.insert( std::make_pair(innerPoly, std::pair< std::list<vtkIdType>, vtkModelFace* >(std::list<vtkIdType>(),NULL)) ).first;
        }
    ...
    if(outerPoly > 0) // -1 and 0 are invalid ids
      ...
      vtkModelFace* face = model->BuildModelFace(numEdges,edge_arr,edge_dir_arr,model->BuildMaterial());
  ...
  std::map< vtkIdType, std::pair< std::list<vtkIdType>, vtkModelFace* > >::iterator polyIter = polyId2InnerLoopsAndFaces.begin();
