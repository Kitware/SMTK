...
bool vtkModelEdgeOperatorClient::Operate(vtkDiscreteModel* Model, vtkSMProxy* ServerModelProxy)
{
  ...
  vtkSMOperatorProxy* OperatorProxy =
    vtkSMOperatorProxy::SafeDownCast(manager->NewProxy("CMBModelGroup", "ModelEdgeOperator"));
  if (!OperatorProxy)
  {
  ...
  vtkSMIntVectorProperty* resproperty =
    vtkSMIntVectorProperty::SafeDownCast(OperatorProxy->GetProperty("LineResolution"));
  resproperty->SetElement(0, this->GetLineResolution());

  if (this->GetIsItemTypeSet())
  {
    vtkSMIntVectorProperty* typeproperty =
      vtkSMIntVectorProperty::SafeDownCast(OperatorProxy->GetProperty("ItemType"));
