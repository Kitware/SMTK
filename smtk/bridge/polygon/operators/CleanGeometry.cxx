  ...
  smtk::model::EntityRefArray& expunged)
{
  ...
  for (SegmentSplitsT::iterator it = sstart; it != sstop; ++it, ++ept0, ++ept1)
    ...
    */
    ...
    if (*ept1 == it->second.low())
      ...
      nn /= 2; // This rounds down, just like we want; no need to swap the middle out for itself when nn is odd.
      ...
      */
      ...
      if (tmp != it)
        ...
        */
        ...
        smtk::model::Vertex splitVert = findOrAddInputModelVertex(mgr, *rit, endpoints, mod, created);
  ...
  if (!splitPts.empty())
    {
    expunged.push_back(curEdge); // The source edge is going away, to be replaced by multiple pieces:

    if (!mod->splitModelEdgeAtModelVertices(mgr, storage, splitVerts, splitPts, created, this->m_debugLevel))
      {
      smtkOpDebug("Could not split " << curEdge.name() << " at " << splitPts.size() << " intersection points.");
      return false;
      }
    for (smtk::model::EntityRefArray::iterator cit = created.begin() + precre; cit != created.end(); ++cit)
      {
...
void CleanGeometry::addVertex(const smtk::model::Vertex& vertex, T& pointMap)
{
  internal::vertex::Ptr storage =
    this->findStorage<internal::vertex>(vertex.entity());
  if (!storage)
    {
...
{
  internal::edge::Ptr storage =
    this->findStorage<internal::edge>(edge.entity());
  if (!storage)
    {
...
void CleanGeometry::addEdgeEndpoints(const smtk::model::Edge& edge, T& pointMap)
{
  ...
  for (auto vit = verts.begin(); vit != verts.end(); ++vit)
    {
    internal::vertex::Ptr storage =
      this->findStorage<internal::vertex>(vit->entity());
    if (!storage)
      {
      smtkErrorMacro(this->log(),
        "Vertex "<< vit->name() << " bounding " << edge.name() << " has no storage.");
      continue;
      }
...
void CleanGeometry::addDeferredSplits(const smtk::model::Edge& edge, T& actions, U& allpoints, U& endpoints, V& created)
{
  ...
  for (++pit; pit != storage->pointsEnd(); ++pit)
    ...
    if (needEdgeSplit(eset, curset, splitPrev, splitNext))
      ...
      if (splitPrev && !alreadySplit)
        {
        vins = findOrAddInputModelVertex(
          mgr, *prevIt, endpoints, mod, created);
        action.m_splitPts.push_back(prevIt);
        action.m_splitVerts.push_back(
          this->findStorage<internal::vertex>(vins.entity()));
        allpoints[*prevIt].insert(vins);
        }
      if (splitNext)
        {
        vins = findOrAddInputModelVertex(
          mgr, *pit, endpoints, mod, created);
...
{
  internal::edge::Ptr storage =
    this->findStorage<internal::edge>(action.m_edge.entity());
  if (!storage)
    {
  ...
  if (!mod->splitModelEdgeAtModelVertices(mgr, storage, action.m_splitVerts, action.m_splitPts, created, this->m_debugLevel))
    {
    smtkOpDebug("Could not split " << action.m_edge.name() << " at " << action.m_splitPts.size() << " intersection points.");
    return false;
    }
...
bool checkBothDirections3(T& p0, T& pt, T& q0, T& p1, T& q1, bool& matchDir)
{
  ...
  if (match && t1 != q1 && t0 == p0 && *t0 == *t1)
    ...
    if (t1 != q1) ++t1; // Since we are periodic.
    if (t0 == pt && t1 == q1)
      {
      matchDir = false;
      return true;
...
bool CleanGeometry::deleteIfDuplicates(T& edgePair, U& modified, U& expunged)
{
  ...
  if (!delset.empty())
    ...
    smtk::model::EntityRef preserve(
      delset.find(edgePair.first) == delset.end() ?
...
smtk::model::OperatorResult CleanGeometry::operateInternal()
{
  ...
  for (iit = inputs.begin(); iit != inputs.end(); ++iit)
    ...
    if (iit->isEdge())
      {
      internal::edge::Ptr storage =
        this->findStorage<internal::edge>(iit->entity());
      if (!storage)
        {
      ...
      if (!mod || (pp && pp != mod))
        {
        smtkErrorMacro(this->log(),
          "Input edge "<< iit->name() << " has no parent model ("
          << mod << ") or a different parent ("
          << pp << ") from other inputs.");
        return this->createResult(smtk::model::OPERATION_FAILED);
        }
      else if (!pp)
        {
      ...
      if (!vv)
        ...
        if (pp && ((mod = vv->parentAs<internal::pmodel>()) != pp))
          {
          smtkErrorMacro(this->log(),
            "Input vertex "<< iit->name() << " has no parent model ("
            << mod << ") or a different parent ("
            << pp << ") from other inputs.");
          return this->createResult(smtk::model::OPERATION_FAILED);
          }
  ...
  smtk::model::EntityRefArray expunged;
    ...
    // IV. Split edges
    smtkDebugMacro(this->log(), segs.size() << " segments in, " << result.size() << " segments out\n");
    for (std::set<smtk::model::Edge>::iterator esit = edgesToSplit.begin(); esit != edgesToSplit.end(); ++esit)
      {
      ...
      if (storage)
        ...
        this->splitEdgeAsNeeded(
          curEdge, storage, result, lkup, revlkup, reslkup, endpoints,
          created, modified, expunged);
        }
      }
