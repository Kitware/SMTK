#include "smtk/bridge/polygon/operators/CreateEdges.h"
...
#include "smtk/bridge/polygon/CreateEdges_xml.h"

namespace smtk {
  namespace bridge {
    namespace polygon {

smtk::model::OperatorResult CreateEdges::operateInternal()
{
  ...
  bool ok = true;
  // These case values match CreateEdges.sbt indices (and enum values):
  switch (method)
    {
  ...
  case 2: // all non-overlapping
      ...
      //     Add an (edge, coedge sign) tuple to a "edge" identified by the given UFE
      // FIXME: Test for self-intersections?
      // FIXME: Deal w/ pre-existing edges?
      }
    break;
  default:
    ok = false;
...
smtkImplementsModelOperator(
  ...
  smtk::bridge::polygon::CreateEdges,
  polygon_create_edges,
  "create edges",
  CreateEdges_xml,
