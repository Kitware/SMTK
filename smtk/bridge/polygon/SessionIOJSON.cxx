...
/**\brief Decode information from \a sessionRec for the given \a mgr.
  ...
  */
int SessionIOJSON::importJSON(smtk::model::ManagerPtr mgr, const smtk::model::SessionPtr& session,
  cJSON* sessionRec, bool loadNativeModels)
{
  smtk::bridge::polygon::Session::Ptr psession = smtk::dynamic_pointer_cast<Session>(session);

  smtk::common::UUIDs models = mgr->entitiesMatchingFlags(smtk::model::MODEL_ENTITY);
  ...
  if (loadNativeModels && idata && idata->type == cJSON_Object)
  {
    ...
    {
      cJSON* trec;
      if (!entry->string || !entry->string[0] || entry->type != cJSON_Object ||
...
/**\brief Encode information into \a sessionRec for the given \a mgr.
  ...
  * Subclasses should return 1 on success and 0 on failure.
  */
int SessionIOJSON::exportJSON(smtk::model::ManagerPtr mgr, const smtk::model::SessionPtr& session,
...
/**\brief Encode information into \a sessionRec for the given \a modelId of the \a mgr.
  ...
  */
int SessionIOJSON::exportJSON(smtk::model::ManagerPtr mgr, const smtk::model::SessionPtr& session,
  const smtk::common::UUIDs& modelIds, cJSON* sessionRec, bool writeNativeModels)
{
  ...
  if (writeNativeModels)
  {
    ...
    for (sit = psession->beginStorage(); sit != psession->endStorage(); ++sit)
    {
      ...
      {
        std::cout << "Ignoring " << sit->second << " id " << sit->second->id().toString()
                  << " parent " << sit->second->parent() << " parent id "
                  << (sit->second->parent() ? sit->second->parent()->id().toString().c_str()
                                            : "nope")
...
cJSON* SessionIOJSON::serializeEdge(internal::EdgePtr edge, const smtk::model::Edge& e)
{
  ...
  {
    cJSON_AddItemToObject(
      result, "parent", cJSON_CreateString(edge->parent()->id().toString().c_str()));
  }
...
cJSON* SessionIOJSON::serializeVertex(internal::VertexPtr vert, const smtk::model::Vertex& v)
{
  ...
  {
    cJSON_AddItemToObject(
      result, "parent", cJSON_CreateString(vert->parent()->id().toString().c_str()));
  }
...
cJSON* SessionIOJSON::serializeIncidentEdgeRecord(const internal::vertex::incident_edge_data& rec)
{
  ...
  {
    cJSON_AddItemToObject(
      result, "cwface", cJSON_CreateString(rec.clockwiseFaceId().toString().c_str()));
  }
  return result;
}
...
internal::pmodel::Ptr SessionIOJSON::deserializeModel(cJSON* record, const smtk::model::Model& m)
{
  ...
  ok &= (item = cJSON_GetObjectItem(record, "origin")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, origin) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "x axis")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, xAxis) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "y axis")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, yAxis) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "z axis")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, zAxis) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "i axis")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, iAxis) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "j axis")) &&
    smtk::io::LoadJSON::getRealArrayFromJSON(item, jAxis) == 3;
  ok &= (item = cJSON_GetObjectItem(record, "model scale")) &&
    smtk::io::LoadJSON::getIntegerArrayFromJSON(item, msData) == 8;
  ok &= (item = cJSON_GetObjectItem(record, "feature size")) && item->type == cJSON_Number &&
    (featureSize = item->valuedouble) > 0;
  ...
  if (ok)
  {
    ...
    result->restoreModel(origin, xAxis, yAxis, zAxis, iAxis, jAxis, featureSize, modelScale);
  }
...
internal::EdgePtr SessionIOJSON::deserializeEdge(cJSON* record, const smtk::model::Edge& e)
{
  ...
  ok &= (item = cJSON_GetObjectItem(record, "points")) &&
    smtk::io::LoadJSON::getIntegerArrayFromJSON(item, ptdata) > 0;
...
internal::VertexPtr SessionIOJSON::deserializeVertex(cJSON* record, const smtk::model::Vertex& v)
{
  ...
  ok &= (item = cJSON_GetObjectItem(record, "t")) && item->type == cJSON_Object;
  ok &= (item = cJSON_GetObjectItem(record, "point")) &&
    smtk::io::LoadJSON::getIntegerArrayFromJSON(item, ptdata) > 0;
...
internal::vertex::incident_edge_data SessionIOJSON::deserializeIncidentEdgeRecord(cJSON* record)
{
  ...
  return (haveEdge && haveFace
      ? internal::vertex::incident_edge_data(edgeId, edgeOut, faceId)
