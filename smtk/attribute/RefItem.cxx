...
RefItem::RefItem(Attribute* owningAttribute, int itemPosition)
  : Item(owningAttribute, itemPosition)
{
}

RefItem::RefItem(Item* inOwningItem, int itemPosition, int mySubGroupPosition)
  : Item(inOwningItem, itemPosition, mySubGroupPosition)
{
}

bool RefItem::setDefinition(smtk::attribute::ConstItemDefinitionPtr adef)
{
  // Note that we do a dynamic cast here since we don't
  // know if the proper definition is being passed
  const RefItemDefinition* def = dynamic_cast<const RefItemDefinition*>(adef.get());
...
std::string RefItem::valueAsString(std::size_t element, const std::string& format) const
{
...
bool RefItem::removeValue(std::size_t element)
{
  //First - are we allowed to change the number of values?
  const RefItemDefinition* def = static_cast<const RefItemDefinition*>(this->definition().get());
  if (def->numberOfRequiredValues() != 0)
...
bool RefItem::setNumberOfValues(std::size_t newSize)
{
...
bool RefItem::assign(ConstItemPtr& sourceItem, unsigned int options)
{
  ...
  for (std::size_t i = 0; i < sourceRefItem->numberOfValues(); ++i)
  {
    if (sourceRefItem->isSet(i))
    {
      ...
      if (!att)
      {
        att = system->copyAttribute(
          sourceRefItem->value(), (options & Item::COPY_MODEL_ASSOCIATIONS) != 0, options);
        if (!att)
        {
          std::cerr << "ERROR: Could not copy Attribute:" << sourceRefItem->value()->name()
                    << " referenced by item: " << sourceItem->name() << "\n";
