  ...
  std::vector<bool> itemRelevances)
{
  ...
  if (att->isRelevant(checkAdvanceLevel, readAccessLevel) != attRelevance)
  {
  ...
  for (int i = 0; i < n; i++)
  {
    if (att->item(i)->isRelevant(checkAdvanceLevel, readAccessLevel) != itemRelevances[i])
    {
...
bool runTests(const attribute::ResourcePtr& attRes, const std::string& prefix)
{
  ...
  // which is set to be ignored
  std::vector<bool> test1 = { false, true, true };
  if (testAttriubuteResource(attRes, false, 0, true, test1))
  {
  ...
  // In this case everyone should fail since the only access level 0 item
  // is set to be ignored
  std::vector<bool> test2 = { false, false, false };
  if (testAttriubuteResource(attRes, true, 0, false, test2))
  {
  ...
  std::vector<bool> test3 = { false, true, false };
  if (testAttriubuteResource(attRes, true, 1, true, test3))
  {
  ...
  std::vector<bool> test4 = { false, true, true };
  if (testAttriubuteResource(attRes, true, 2, true, test4))
  {
  ...
  cats.insert("B");
  attRes->setActiveCategories(cats);
  std::vector<bool> test5 = { false, false, false };
  if (testAttriubuteResource(attRes, false, 0, false, test5))
  {
...
void setupAttributeResource(attribute::ResourcePtr& attRes)
{
  ...
  VoidItemDefinitionPtr vItemDef = A->addItemDefinition<VoidItemDefinition>("i0");
