...
void verifyDefault()
{
  ...
  sa::ResourcePtr resptr = sa::Resource::create();
  sa::Resource& resource(*resptr.get());
  sa::DefinitionPtr attDef = resource.createDefinition("test-datetime");
  attDef->addItemDefinition(dtDef);
  sa::AttributePtr att = resource.createAttribute(attDef);
  ...
  sa::AttributePtr attSet = resource.createAttribute(attDef);
...
void verifySerialize()
{
  ...
  sa::AttributePtr att = outputResource.createAttribute(attDef);
  ...
  // Write to string
  bool writeError = writer.writeContents(outputResptr, contents, logger);
  if (writeError)
  {
  ...
  sa::DefinitionPtr inputDef = inputResource.findDefinition("test-att");
  ...
  // Check attribute
  sa::AttributePtr inputAtt = inputResource.findAttribute(att->name());
