...
def Delete(ents, **kwargs):
    ...
    if 'delete_dependents' in kwargs and kwargs['delete_dependents']:
        op.findVoid('delete dependents', int(
            smtk.attribute.ALL_CHILDREN)).setIsEnabled()
    res = op.operate()
    SetLastResult(res)
    PrintResultLog(res)
...
def Scale(bodies, factor, **kwargs):
    ...
    method = ScaleType.UNIFORM if isinstance(
        factor, Number) else ScaleType.PER_AXIS
...
def CreateVertices(pt, model, **kwargs):
    ...
    for ix in range(numPts):
        ...
        SetVectorValue(xx, pt[ix][0:numCoordsPerPoint] +
...
def Sweep(stuffToSweep, method=SweepType.EXTRUDE, **kwargs):
    ...
    if method == SweepType.EXTRUDE:
        ...
        if 'draft_angle' in kwargs:
            draft_angle = swp.findAsDouble(
                'draft angle').setValue(0, kwargs['draft_angle'])
        if 'draft_type' in kwargs:
            draft_type = swp.findAsInt('draft type').setValue(
                0, kwargs['draft_type'])
        ...
        if 'sweep_angle' in kwargs:
            angl = swp.findAsDouble('sweep angle').setValue(
                0, kwargs['sweep_angle'])
        ...
        if 'helix_angle' in kwargs:
            angl = swp.findAsDouble('helix angle').setValue(
                0, kwargs['helix_angle'])
        if 'pitch' in kwargs:
            pitch = swp.findAsDouble('pitch').setValue(0, kwargs['pitch'])
        if 'handedness' in kwargs:
            angl = swp.findAsInt('handedness').setValue(
                0, kwargs['handedness'])
    res = swp.operate()
    SetLastResult(res)
    PrintResultLog(res)
