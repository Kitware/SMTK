...
void qtAttributeRefItem::refreshUI(QComboBox* comboBox)
{
  ...
  if (curIdx > 0) // index 0 is None
  {
    ...
    if (curIdx == comboBox->count() - 1) // create New attribute
    {
      ...
      if (defTypes.count() > 0)
      {
        std::string attName = attCollection->createUniqueName(defTypes[0].toStdString());
          ...
          !attDialog.attributeName().isEmpty())
        {
          ...
          attPtr = attCollection->createAttribute(
    ...
    {
      attPtr = attCollection->findAttribute(comboBox->currentText().toStdString());
    }
