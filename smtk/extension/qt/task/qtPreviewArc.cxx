...
void qtPreviewArc::setPredecessor(qtBaseTaskNode* predecessor)
{
  ...
  {
    QObject::disconnect(
      m_predecessor, &qtBaseTaskNode::nodeMoved, this, &qtPreviewArc::updateArcPoints);
    QObject::disconnect(
      m_predecessor, &qtBaseTaskNode::nodeResized, this, &qtPreviewArc::updateArcPoints);
    if (m_operation)
    {
      m_operation->parameters()->disassociate(m_predecessor->task()->shared_from_this());
    }
  }
  ...
  if (m_predecessor)
  {
    m_canAssociatePredecessor = m_operation
      ? m_operation->parameters()->associate(m_predecessor->task()->shared_from_this())
      : false;
    QObject::connect(
      m_predecessor, &qtBaseTaskNode::nodeMoved, this, &qtPreviewArc::updateArcPoints);
    QObject::connect(
...
void qtPreviewArc::setSuccessor(qtBaseTaskNode* successor, bool updatePoints)
{
  ...
  {
    QObject::disconnect(
      m_successor, &qtBaseTaskNode::nodeMoved, this, &qtPreviewArc::updateArcPoints);
    QObject::disconnect(
      m_successor, &qtBaseTaskNode::nodeResized, this, &qtPreviewArc::updateArcPoints);
    if (m_operation)
    {
      auto successorItem = m_operation ? m_operation->parameters()->findComponent("to") : nullptr;
      if (successorItem)
      {
        successorItem->unset();
      }
  ...
  if (m_successor)
  {
    ...
    m_canAssociateSuccessor =
      successorItem ? successorItem->setValue(m_successor->task()->shared_from_this()) : false;
    QObject::connect(m_successor, &qtBaseTaskNode::nodeMoved, this, &qtPreviewArc::updateArcPoints);
    QObject::connect(
      m_successor, &qtBaseTaskNode::nodeResized, this, &qtPreviewArc::updateArcPoints);
  }
...
bool qtPreviewArc::setArcType(smtk::string::Token arcType, const std::string& operationType)
{
  if (arcType == m_arcType && m_arcOperation == operationType)
  {
    // Do nothing.
    return false;
  }
...
void qtPreviewArc::createOperation()
{
  ...
  if (m_operationManager && !m_arcOperation.empty())
  {
    ...
    if (m_operation)
    {
      ...
      if (m_successor)
      {
        auto successorItem = m_operation->parameters()->findComponent("to");
        if (successorItem)
        {
          m_canAssociateSuccessor =
            successorItem->setValue(m_successor->task()->shared_from_this());
        }
      }
