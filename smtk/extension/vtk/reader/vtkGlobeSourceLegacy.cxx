...
vtkStandardNewMacro(vtkGlobeSourceLegacy);

// 0=NE, 1=SE, 2=SW, 3=NW

//----------------------------------------------------------------------------
vtkGlobeSourceLegacy::vtkGlobeSourceLegacy()
{
  VTK_LEGACY_BODY(vtkGlobeSourceLegacy::vtkGlobeSourceLegacy, "VTK 8.2");
...
//----------------------------------------------------------------------------
void vtkGlobeSourceLegacy::ComputeGlobePoint(
...
//----------------------------------------------------------------------------
void vtkGlobeSourceLegacy::ComputeLatitudeLongitude(double* x, double& theta, double& phi)
{
...
//----------------------------------------------------------------------------
void vtkGlobeSourceLegacy::AddPoint(double theta, double phi, double radius, vtkPoints* newPoints,
  vtkFloatArray* newNormals, vtkFloatArray* newLongitudeArray, vtkFloatArray* newLatitudeArray,
  vtkDoubleArray* newLatLongArray)
{
  double x[3], n[3];

  vtkGlobeSourceLegacy::ComputeGlobePoint(theta, phi, radius, x, n);
  ...
  vtkInformationVector** /*inputVector*/, vtkInformationVector* outputVector)
{
  ...
  {
    this->CurtainHeight = (this->EndLongitude - this->StartLongitude) * this->Radius / 3600.0;
  }
  ...
  deltaLatitude =
    (this->EndLatitude - this->StartLatitude) / static_cast<double>(this->LatitudeResolution - 1);
  ...
  for (j = 0; j < this->LatitudeResolution; j++)
  {
    ...
    for (i = 0; i < this->LongitudeResolution; i++)
    {
      ...
      this->AddPoint(theta, phi, this->Radius, newPoints, newNormals, newLongitudeArray,
        newLatitudeArray, newLatLongArray);
    }
    this->UpdateProgress(0.10 + 0.50 * j / static_cast<float>(this->LatitudeResolution));
  }
  ...
  for (i = 0; i < this->LongitudeResolution; i++)
  {
    ...
    this->AddPoint(theta, phi, this->Radius - this->CurtainHeight, newPoints, newNormals,
  ...
  for (j = 1; j < this->LatitudeResolution; ++j)
  {
    ...
    this->UpdateProgress(0.70 + 0.30 * j / static_cast<double>(this->LatitudeResolution));
  }

  // Create curtain quads.
  vtkIdType curtainPointId = this->LongitudeResolution * this->LatitudeResolution;
...
//----------------------------------------------------------------------------
void vtkGlobeSourceLegacy::PrintSelf(ostream& os, vtkIndent indent)
{
