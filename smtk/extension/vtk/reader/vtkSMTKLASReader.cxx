...
int vtkSMTKLASReader::ReadHeaderBlock()
{
  ...
  if (this->Header.VersionMajor != 1 ||
    (this->Header.VersionMinor != 1 && this->Header.VersionMinor != 2))
  {
    ...
    this->Header.PointDataFormat > 4)
  {
    vtkErrorMacro("Invalid data format (" << static_cast<int>(this->Header.PointDataFormat)
                                          << ") / record lengh ("
                                          << this->Header.PointDataRecordLength << ") combo!");
    return READ_ERROR;
  }
...
int vtkSMTKLASReader::ReadPoints(vtkMultiBlockDataSet* output)
{
  ...
  for (vtkTypeUInt32 ptIndex = 0; ptIndex < this->Header.NumberOfPointRecords; ptIndex++)
  {
    ...
    {
      this->UpdateProgress(
        static_cast<double>(ptIndex) / static_cast<double>(this->Header.NumberOfPointRecords));
    ...
    if (!pieceInfo[classification].PolyData)
    {
      ...
      if (!this->ScanMode)
      {
        ...
        unsigned long numberOfValuesEstimate = 2 +
          this->PointRecordsPerClassification[classification] / pieceInfo[classification].ReadRatio;
