...
public:
  ...
  PolyFileModelBuilder() { this->NextPointId = 0; }
};
...
public:
  ...
  void NumberOfNodes(vtkIdType nn) { this->Points->SetNumberOfPoints(nn); }
  ...
  void Finalize(vtkPolyData* polyOutput, vtkPolyData* facetHoleOutput,
    vtkPolyData* volumeHoleOutput, vtkPolyData* regionOutput)
  {
    ...
    {
      polyOutput->GetFieldData()->AddArray(this->FacetBoundaryMarkers.GetPointer());
    }
...
bool vtkPolyFileReader::ReadNodes(std::istream& in, vtkIdType numPts, const std::string& nodeSpec,
  int& dimension, int& numAttribs, ErrorReporter& err)
{
  bool ok;
  dimension = 0;
  numAttribs = 0;
...
template <typename ErrorReporter>
bool vtkPolyFileReader::ReadSegments(std::istream& in, int& bdyMarkers, ErrorReporter& err)
{
  ...
  for (int i = 0; i < numSegments; ++i)
  {
    ...
    if (bdyMarkers && nv < 1)
    {
      ...
      {
        err.Report(
          in, static_cast<int>(in.tellg()) - 1, in.tellg(), VTK_POLYFILE_MISSING_SEGMENT_BDY);
      }
...
bool vtkPolyFileReader::ReadSimpleFacets(
  std::istream& in, int /*dimension*/, int& bdyMarkers, ErrorReporter& err)
{
  // Lines are special in this section as some numbers are mandatory de jure but optional de facto.
  bool ok;
  ...
  for (int i = 0; i < numFacets; ++i)
  {
    ...
    if (ok && numCorners)
    {
      ...
      if (bdyMarkers && nv < 1)
      {
        if (++numBdyWarnings < 10)
        {
          err.Report(in, static_cast<int>(in.tellg()), in.tellg(), VTK_POLYFILE_MISSING_FACET_BDY);
        }
        else if (numBdyWarnings < 11)
        {
...
void vtkPolyFileReader::ReadFile(
  std::istream& in, int isSimpleMesh, ErrorReporter& err, const std::string& nodeFileName)
{
  ...
  vtkInformation* /*request*/, vtkInformationVector** /*inVec*/, vtkInformationVector* outVec)
{
  ...
  vtkPolyData* polyOutput =
    vtkPolyData::SafeDownCast(polyOutInfo->Get(vtkDataObject::DATA_OBJECT()));
  vtkInformation* facetHoleOutInfo = outVec->GetInformationObject(1);
  vtkPolyData* facetHoleOutput =
    vtkPolyData::SafeDownCast(facetHoleOutInfo->Get(vtkDataObject::DATA_OBJECT()));
  vtkInformation* volumeHoleOutInfo = outVec->GetInformationObject(2);
  vtkPolyData* volumeHoleOutput =
    vtkPolyData::SafeDownCast(volumeHoleOutInfo->Get(vtkDataObject::DATA_OBJECT()));
  vtkInformation* regionOutInfo = outVec->GetInformationObject(3);
  vtkPolyData* regionOutput =
    vtkPolyData::SafeDownCast(regionOutInfo->Get(vtkDataObject::DATA_OBJECT()));
  ...
  nodeFileName =
    vtksys::SystemTools::GetParentDirectory(this->FileName) + "/" + nodeFileName + ".node";
