  ...
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  this->StripLines(initialLines.GetPointer(), strippedLines.GetPointer(), lengths.GetPointer());
...
void vtkCleanPolylines::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "Use Relative Line Length: " << (this->UseRelativeLineLength ? "On" : "Off")
     << endl;
}

void vtkCleanPolylines::StripLines(vtkPolyData* input, vtkPolyData* result, vtkDoubleArray* lengths)
{
  ...
  vtkPolyData* input, vtkDoubleArray* lengths, vtkPolyData* result, vtkDoubleArray* newLengths)
{
  ...
  for (cell = 0; cell < numCells; cell++)
  {
    ...
    if (cellCounts[0] == 1)
    {
      this->TraversePolyLine(testPoints[0], cell, input, lengths, marks, ids.GetPointer(), &l);
      plines->InsertNextCell(ids.GetPointer());
