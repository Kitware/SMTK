...
public:
  ...
  bool ScopeImporterForResource();

  /// Set the import mode to only import resources of a given type. The resource
  /// is identified by its unique name.
  bool ScopeImporterForResource(const std::string&);

  /// Set the import mode to only import resources of a given type. THe resource
  /// is identified by its type index.
  bool ScopeImporterForResource(const smtk::resource::Resource::Index&);
...
template <typename ResourceType>
bool vtkSMTKModelImporter::ScopeImporterForResource()
{
  return this->ScopeImporterForResource(std::type_index(typeid(ResourceType)).hash_code());
}
