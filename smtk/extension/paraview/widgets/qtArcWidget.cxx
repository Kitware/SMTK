...
qtArcWidget::qtArcWidget(QWidget *parentWdg)
  ...
  this->connect(this->Internals->Visibility, SIGNAL(toggled(bool)),
  ...
  QObject::connect(this->Internals->Closed, SIGNAL(toggled(bool)), this,
                   SLOT(closeLoop(bool)));

  QObject::connect(this->Internals->Delete, SIGNAL(clicked()), this,
                   SLOT(deleteAllNodes()));

  QObject::connect(this->Internals->EditMode, SIGNAL(toggled(bool)), this,
                   SLOT(updateMode()));
  QObject::connect(this->Internals->ModifyMode, SIGNAL(toggled(bool)), this,
                   SLOT(updateMode()));
  QObject::connect(this->Internals->Finished, SIGNAL(clicked()), this,
                   SLOT(finishContour()));
  QPushButton *finishButton = this->Internals->Finished;
  QPalette applyPalette = finishButton->palette();
  applyPalette.setColor(QPalette::Active, QPalette::Button,
                        QColor(161, 213, 135));
  applyPalette.setColor(QPalette::Inactive, QPalette::Button,
                        QColor(161, 213, 135));
  finishButton->setPalette(applyPalette);
  finishButton->setDefault(true);

  this->Internals->ClosedLoopConnect->Connect(
      this->widgetProxy(), vtkCommand::EndInteractionEvent, this,
