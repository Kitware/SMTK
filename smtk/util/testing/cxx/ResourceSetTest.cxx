...
// Basic tests for smtk::util::ResourceSet
...
int main(int /* argc */, const char* /* argv */[])
{
  ...
  bool result;
  unsigned n;
  smtk::util::ResourceSet resourceSet;
  ...
  result = resourceSet.addResource(manager1, "manager1-different-id", "",
                                   smtk::util::ResourceSet::SCENARIO);
  n = resourceSet.numberOfResources();
  if (!result)
    {
    std::cerr << "addResource() call failed" << std::endl;
  ...
  smtk::util::ResourceSet::ResourceState state;
  std::string link;
  ...
  if (!result)
    ...
    {
    if (rtype != smtk::util::Resource::ATTRIBUTE)
      {
      std::cerr << "Incorrect resource type " << rtype
                << ", should be " << smtk::util::Resource::ATTRIBUTE
                << std::endl;
      status += 1;
      }
    if (role != smtk::util::ResourceSet::INSTANCE)
      {
      std::cerr << "Incorrect resource role " << role
                << ", should be " << smtk::util::ResourceSet::INSTANCE
                << std::endl;
      status += 1;
      }
    if (state != smtk::util::ResourceSet::LOADED)
      {
      std::cerr << "Incorrect resource state " << state
                << ", should be " << smtk::util::ResourceSet::LOADED
                << std::endl;
      status += 1;
      }
    if (link != "path2")
