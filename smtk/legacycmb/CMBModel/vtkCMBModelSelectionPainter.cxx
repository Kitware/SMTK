/*=========================================================================
  ...
  Module:    vtkCMBModelSelectionPainter.cxx
                                         ...
                                         bool forceCompileOnly)
{
  ...
  if (!inputModel || !this->DelegatePainter)
    {
    this->vtkPainter::RenderInternal(renderer, actor, typeflags,
                                     forceCompileOnly);
    return;
    }
  ...
  for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextItem())
    ...
    entity = vtkModelGeometricEntity::SafeDownCast(
      inputModel->GetEntityObjectByFlatIndex(curr_idx));
  ...
  iter->Delete();
  RenderEntities(facesList, selector, this->Information, this->DelegatePainter,
    this->OutputData, renderer, actor, typeflags, forceCompileOnly);
  RenderEntities(edgesList, selector, this->Information, this->DelegatePainter,
    this->OutputData, renderer, actor, typeflags, forceCompileOnly);
  RenderEntities(vertsList, selector, this->Information, this->DelegatePainter,
    this->OutputData, renderer, actor, typeflags, forceCompileOnly);
}
