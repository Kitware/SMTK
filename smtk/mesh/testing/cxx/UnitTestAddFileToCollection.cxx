...
//----------------------------------------------------------------------------
void verify_append_boundary_to_dirichlet()
{
  smtk::mesh::ManagerPtr manager = smtk::mesh::Manager::create();
  smtk::mesh::CollectionPtr c = smtk::io::ImportMesh::onlyDirichlet(first_mesh_path(), manager);
  test( c->isValid(), "collection should be valid");
  ...
  std::size_t secondNumMesh = 0;
  {
  smtk::mesh::CollectionPtr c2 = smtk::io::ImportMesh::onlyBoundary(first_mesh_path(),manager);
  test( !c2->isValid(), "no boundary in this file");
  }

  //now append a second file and verify the number of meshes is correct
  bool result = smtk::io::ImportMesh::addBoundaryToCollection(first_mesh_path(), c);
  test( !result, "no boundary sets in this file");
  test( c->isValid(), "collection after import should still be valid");
...
//----------------------------------------------------------------------------
int UnitTestAddFileToCollection(int argc, char** argv)
{
  ...
  //attempt to append Boundary set to a collection that only has Dirichlet
  verify_append_boundary_to_dirichlet();
