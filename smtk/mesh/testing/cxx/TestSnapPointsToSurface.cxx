...
int TestSnapPointsToSurface(int argc, char* argv[])
{
  (void)argc;
  (void)argv;
...

  {
    ...
    createInstances->parameters()->findString("placement rule")->setDiscreteIndex(1);
    auto xAxis = std::dynamic_pointer_cast<smtk::attribute::DoubleItem>(
      createInstances->parameters()->findGroup("volume of interest")->item(0, 0));
    xAxis->setValue(0, 57.);
    xAxis->setValue(1, 60.);
    auto yAxis = std::dynamic_pointer_cast<smtk::attribute::DoubleItem>(
      createInstances->parameters()->findGroup("volume of interest")->item(1, 0));
    yAxis->setValue(0, 2.);
    yAxis->setValue(1, 5.);
    auto zAxis = std::dynamic_pointer_cast<smtk::attribute::DoubleItem>(
      createInstances->parameters()->findGroup("volume of interest")->item(2, 0));
  ...
  for (const auto& instance : instances)
  {
    ...
    for (std::size_t i = 2; i < coords.size(); i += 3)
    {
      if (coords[i] >= 0.1)
      {
        std::cerr << "coordinate " << i / 3 << " (" << coords[i - 2] << ", " << coords[i - 1]
                  << ", " << coords[i] << ") did not snap" << std::endl;
        return 1;
      }
    }
  }
