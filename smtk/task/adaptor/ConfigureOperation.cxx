  ...
  : Superclass(config, from, to)
{
  ...
  std::cout << __FILE__ << ":" << __LINE__ << " " << std::hex << &m_taskObserver << std::dec
            << std::endl;
}
...
bool ConfigureOperation::buildInternalData(const Configuration& config)
{
  ...
  // smtk::operation::Operation::Parameters parameters = operation->parameters();
...
void ConfigureOperation::updateOperation() const
{
...
void ConfigureOperation::updateOperation(
  const smtk::task::FillOutAttributes::AttributeSet& attSet,
  const ParameterSet& paramSet) const
{
  // std::cout << __FILE__ << ":" << __LINE__ << " " << std::endl;
