...
#include "smtk/io/XmlV1StringWriter.h"
...
void XmlV1StringWriter::processModelInfo()
{
  ...
  smtk::model::Manager::Ptr refModel = this->m_manager.refModelMgr();
...
//----------------------------------------------------------------------------
std::string XmlV1StringWriter::encodeModelEntityMask(smtk::model::BitFlags m)
{
  std::string s;
  if (m & smtk::model::GROUP_ENTITY)
    {
    s.append("g");
    }
  if (m & smtk::model::MODEL_DOMAIN)
    {
    s.append("m");
    }
  if (m & smtk::model::VOLUME)
    {
    s.append("r");
    }
  if (m & smtk::model::FACE)
    {
    s.append("f");
    }
  if (m & smtk::model::EDGE)
    {
    s.append("e");
    }
  if (m & smtk::model::VERTEX)
    {
    s.append("v");
    }
  return s;
