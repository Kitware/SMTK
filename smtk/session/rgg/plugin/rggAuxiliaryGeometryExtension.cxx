...
using smtk::session::rgg::RGGType;
...
{
  rggAuxiliaryGeometryExtension::ensureCache();
  if (rggEntity.stringProperty("rggType")[0] == SMTK_SESSION_RGG_PIN)
  {
  ...
  else if (rggEntity.stringProperty("rggType")[0] == SMTK_SESSION_RGG_DUCT)
  {
    if (rggEntity.auxiliaryGeometries().size() == 0)
    {
  ...
  const AuxiliaryGeometry& pin, bool genNormals)
{
  ...
  for (size_t j = 0; j < numParts; ++j)
  {
    ...
    {
      std::string subName = pin.name() + SMTK_SESSION_RGG_PIN_SUBPART + std::to_string(j) +
        SMTK_SESSION_RGG_PIN_LAYER + std::to_string(k);
      if (isMaterialSet && k == numLayers)
      {
        subName = pin.name() + SMTK_SESSION_RGG_PIN_SUBPART + std::to_string(j) +
          SMTK_SESSION_RGG_PIN_MATERIAL;
      }
  ...
  const AuxiliaryGeometry& duct, bool /*genNormals*/)
{
  ...
  for (size_t i = 0; i < numSegs; i++)
  {
    ...
    for (size_t k = 0; k < numLayers; k++)
    {
      std::string subName = duct.name() + SMTK_SESSION_RGG_DUCT_SEGMENT + std::to_string(i) +
        SMTK_SESSION_RGG_DUCT_LAYER + std::to_string(k);
