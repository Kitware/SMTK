...
public:
  ...
  virtual std::vector<int> pointIds(int modelEntityId, PointClosure closure, ApiStatus& status);

  /// Returns the grid point ids for a specified grid cell id.
  //  The points are ordered using the VTK convention.
  virtual std::vector<int> cellPointIds(int gridCellId, ApiStatus& status);

  /// Returns the dimensional coordinates of a specified grid point id.
  virtual std::vector<double> pointLocation(int gridPointId, ApiStatus& status);
  ...
  virtual std::string nodeElemSetClassification(int modelEntityId, ApiStatus& status);

  /// Returns the classification id for the mesh side set for a specified
  //  model entity. The function applies to MOAB-style grids.
  virtual std::string sideSetClassification(int modelEntityId, ApiStatus& status);
