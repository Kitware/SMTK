...
int vtkCompleteShells::RequestData(vtkInformation* /*request*/, vtkInformationVector** inputVector,
  vtkInformationVector* outputVector)
{
  ...
  {
    vtkErrorMacro(
      "Cell Normals MUST be specifed on the input (probably via runing vtkMasterPolyDataNormals)");
    return 0;
  }
  ...
  vtkDataArray* genericModelFaceArray = input->GetCellData()->GetArray(
    this->ModelFaceArrayName ? this->ModelFaceArrayName : ModelParserHelper::GetModelFaceTagName());
  ...
  for (cellIndex = 0; cellIndex < numCells; cellIndex++)
  {
    ...
    this->FindRegionEdge(input, cellNormals, regionArray, modelFaceArray, cellIndex, visited);
  }
  ...
  if (this->DetectAndFixSubmergedSolids)
  {
    ...
    if (!singleFaceRegions.empty())
    {
      ...
      for (regionIter = singleFaceRegions.begin(); regionIter != singleFaceRegions.end();
           regionIter++)
      {
        this->FindClosestEnclosingRegion(regionIter->first, regionIter->second, input, cellNormals,
          regionArray, modelFaceArray, cellLocator);
      }
    }
  }
  ...
  modelFaceRegionsArray->SetName(ModelParserHelper::GetModelFaceRegionsMapString());
...
void vtkCompleteShells::FindRegionEdge(vtkPolyData* input, vtkDataArray* cellNormals,
  vtkIntArray* regionArray, vtkIdTypeArray* modelFaceArray, vtkIdType cellIndex, char* visited)
{
  ...
  while (!cellSearchList.empty())
  {
    ...
    for (vtkIdType ptIndex = 0; ptIndex < npts; ptIndex++)
    {
      ...
      vtkIdType cellIdToFixHole = this->FindHoleFillingModelFace(
        input, cellNormals, currentCellId, neighborIds, pts, ptIndex, otherPtIndex);
      ...
      int otherRegionInMap = this->Internals->FaceToRegionsMap[modeFaceId].second;
  ...
  for (vtkIdType j = 0; j < neighborIds->GetNumberOfIds(); j++)
  {
    ...
    for (vtkIdType i = 0; i < neighborNPts; i++)
    {
      ...
      double planeEvalResult =
        vtkPlane::Evaluate(cellNormal, cellOrigin, input->GetPoints()->GetPoint(neighborPts[i]));
      ...
      if ((planeEvalResult < 0 && anotherTest < 0) || (planeEvalResult > 0 && anotherTest > 0))
      {
        // not a candidate
        break;
      }
...
void vtkCompleteShells::FindClosestEnclosingRegion(int regionId, vtkIdType modelFaceId,
  ...
  vtkIdTypeArray* modelFaceArray, vtkCellLocator* locator)
{
  ...
  double locatorSeedPt[6][3] = { { bounds[0], midY, midZ }, { bounds[1], midY, midZ },
    { midX, bounds[2], midZ }, { midX, bounds[3], midZ }, { midX, midY, bounds[4] },
    { midX, midY, bounds[5] } };
  double* inputBounds = input->GetBounds();
  double searchDirection[6][3] = { { inputBounds[0] - inputBounds[1], 0, 0 },
    { inputBounds[1] - inputBounds[0], 0, 0 }, { 0, inputBounds[2] - inputBounds[3], 0 },
    { 0, inputBounds[3] - inputBounds[2], 0 }, { 0, 0, inputBounds[4] - inputBounds[5] },
  ...
  for (int i = 0; i < 6; i++)
  {
    ...
    {
      if (input->GetCell(candidateCells->GetId(j))
            ->IntersectWithLine(locatorSeedPt[i], endPt, 0.0, t, x, pcoords, subId))
      {
        intersectingCellMap.insert(std::pair<double, vtkIdType>(t, candidateCells->GetId(j)));
      }
    }
    ...
    for (cellIter = intersectingCellMap.begin(); cellIter != intersectingCellMap.end(); cellIter++)
    {
  ...
  {
    vtkWarningMacro(
      "Unaniomous decision (of 6) on interior test of solid not obtained (region, count): ("
      << regionId << ", " << maxCount << ")");
  }
...
void vtkCompleteShells::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent
     << "DetectAndFixSubmergedSolids: " << (this->DetectAndFixSubmergedSolids ? "On" : "Off")
     << endl;
  os << indent
     << "MinimumSubmergedVoteCountToAvoidWarning: " << this->MinimumSubmergedVoteCountToAvoidWarning
     << endl;
}
