...
// .NAME vtkCMBModelReadOperation -
// .SECTION Description
// Front end for the readers.  Reads in a vtkPolyData and then figures
// out how to parse that vtkPolyData.

#ifndef __smtkdiscrete_vtkCMBModelReadOperation_h
#define __smtkdiscrete_vtkCMBModelReadOperation_h
...
class SMTKDISCRETESESSION_EXPORT vtkCMBModelReadOperation : public vtkObject
{
public:
  static vtkCMBModelReadOperation* New();
  vtkTypeMacro(vtkCMBModelReadOperation, vtkObject);
  ...
  void Operate(vtkDiscreteModelWrapper* ModelWrapper, smtk::session::discrete::Session* session);

  // Description:
  // Load the file into Model.
  void Read(vtkDiscreteModel* model, smtk::session::discrete::Session* session);
...
protected:
  vtkCMBModelReadOperation();
  virtual ~vtkCMBModelReadOperation();
...
private:
  ...
  vtkCMBModelReadOperation(const vtkCMBModelReadOperation&); // Not implemented.
  void operator=(const vtkCMBModelReadOperation&);           // Not implemented.
