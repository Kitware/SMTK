...
bool vtkCMBModelWriterV2::Write(vtkDiscreteModel* Model, smtk::session::discrete::Session* session)
{
  ...
  vtkIdType index = 0;
  for (CellIterator i = mesh.CellsBegin(); i != mesh.CellsEnd(); ++i, ++index)
  {
...
void vtkCMBModelWriterV2::SetModelEntityGroupData(vtkDiscreteModel* Model, vtkPolyData* Poly)
{
  ...
  for (EntityGroups->Begin(); !EntityGroups->IsAtEnd(); EntityGroups->Next())
  {
    ...
    if (NumberOfModelEntities > 0)
    {
      ...
      {
        vtkIdType EntityId =
          vtkModelEntity::SafeDownCast(EntitiesIter->GetCurrentItem())->GetUniquePersistentId();
...
void vtkCMBModelWriterV2::SetUUIDData(
  vtkDiscreteModel* model, vtkPolyData* poly, smtk::session::discrete::Session* session)
{
  ...
  for (itit->Begin(); !itit->IsAtEnd(); itit->Next())
    entities.push_back(itit->GetCurrentItem());
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetMaterialUUIDsString(), session);
  // Groups
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelGroupUUIDsString(), session);
  // Regions
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelRegionUUIDsString(), session);
  // Faces
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelFaceUUIDsString(), session);
  // Edges
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelEdgeUUIDsString(), session);
  // Vertices
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelVertexUUIDsString(), session);
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelShellUUIDsString(), session);
  // Loop
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelLoopUUIDsString(), session);
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelFaceUseUUIDsString(), session);
  // Edge uses
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelEdgeUseUUIDsString(), session);
  // Vertex uses
  entities.clear();
  ...
  itit->Delete();
  this->SetModelItemUUIDs(
    model, poly, entities, ModelParserHelper::GetModelVertexUseUUIDsString(), session);
}

void vtkCMBModelWriterV2::SetModelEntityData(
  vtkPolyData* Poly, std::vector<vtkModelEntity*>& ModelEntities, const char* BaseArrayName)
{
...
void vtkCMBModelWriterV2::SetModelItemUUIDs(vtkDiscreteModel* model, vtkPolyData* poly,
  std::vector<vtkModelItem*>& items, const char* arrayName,
  smtk::session::discrete::Session* session)
{
  if (session)
  {
