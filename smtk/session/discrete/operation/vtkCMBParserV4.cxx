  ...
  smtk::session::discrete::Session* vtkNotUsed(session))
{
  ...
  vtkIdTypeArray* vertexPointIds =
    this->NewIdTypeArray(masterFieldData->GetArray(ModelParserHelper::GetVertexPointIdString()));
  ...
  for (vtkIdType i = 0; i < FaceIdsArray->GetNumberOfTuples(); i++)
  {
    ...
    vtkDiscreteModelFace* face = nullptr;
    if (FaceMaterials && FaceMaterials->GetValue(i) >= 0)
    {
      ...
      if (EdgeCounter == 0)
      {
        face = vtkDiscreteModelFace::SafeDownCast(
          Model->BuildModelFace(EdgeCounter, nullptr, nullptr, Material));
      }
      else
      {
        face = vtkDiscreteModelFace::SafeDownCast(
    ...
    else
    {
      ...
      {
        face = vtkDiscreteModelFace::SafeDownCast(
          Model->BuildModelFace(EdgeCounter, nullptr, nullptr, FaceIdsArray->GetValue(i)));
      }
      else
      {
        face = vtkDiscreteModelFace::SafeDownCast(
          Model->BuildModelFace(EdgeCounter, &Edges[0], &EdgeDirs[0], FaceIdsArray->GetValue(i)));
      }
    }
  ...
  if (EntityGroupIds)
  {
    ...
    for (vtkIdType i = 0; i < EntityGroupIds->GetNumberOfTuples(); i++)
    {
      ...
      if (numEntities)
      {
        ...
        {
          vtkModelEntity* Entity2 =
            Model->GetModelEntity(itemType, GroupedEntityIds->GetValue(ArrayCounter));
          ArrayCounter++;
  ...
  if (pointMapArray || cellMapArray || canonicalSideArray)
  {
    if (pointMapArray && cellMapArray && canonicalSideArray)
    {
      this->SetAnalysisGridInfo(Model, pointMapArray, cellMapArray, canonicalSideArray);
    }
    else
    {
  ...
  if (masterPoints->GetNumberOfArrays() != 0)
  {
    ...
    {
      vtkDebugMacro("Point data array name is " << masterPoints->GetArrayName(ii));
    }
  }
  if (masterCells->GetNumberOfArrays() != 0)
  {
    ...
    {
      vtkDebugMacro("Cell data array name is " << masterCells->GetArrayName(ii));
    }
  }
