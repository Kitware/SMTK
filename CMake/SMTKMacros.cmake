...
```
  ...
  * `CLASS_NAME`: The name of the UI view class.
  * `VIEW_NAME`: The name of the view which will be registered.
  * `INTERFACES`: The name of the generated interface.
  * `SOURCES`: The source files generated by the interface.
  * `HEADER_PATH`: The path to the mentioned class header.
#]==]
function (smtk_plugin_add_ui_view)
  cmake_parse_arguments(_smtk_ui_view
    ""
    "CLASS_NAME;VIEW_NAME;INTERFACES;SOURCES;HEADER_PATH"
    ""
    ${ARGN})

  if (_smtk_ui_view_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR
      "Unparsed arguments for paraview_plugin_add_action_group: "
      "${_smtk_ui_view_UNPARSED_ARGUMENTS}")
  endif ()

  if (NOT DEFINED _smtk_ui_view_CLASS_NAME)
    message(FATAL_ERROR
      "The `CLASS_NAME` argument is required.")
  endif ()

  if (NOT DEFINED _smtk_ui_view_VIEW_NAME)
    message(FATAL_ERROR
      "The `VIEW_NAME` argument is required.")
  endif ()

  if (NOT DEFINED _smtk_ui_view_INTERFACES)
    message(FATAL_ERROR
      "The `INTERFACES` argument is required.")
  endif ()

  if (NOT DEFINED _smtk_ui_view_SOURCES)
    message(FATAL_ERROR
      "The `SOURCES` argument is required.")
  endif ()

  configure_file(
    "${_SMTKMacros_cmake_dir}/qtSMTKViewImplementation.h.in"
    "${CMAKE_CURRENT_BINARY_DIR}/${_smtk_ui_view_CLASS_NAME}Implementation.h"
    @ONLY)
  configure_file(
    "${_SMTKMacros_cmake_dir}/qtSMTKViewImplementation.cxx.in"
    "${CMAKE_CURRENT_BINARY_DIR}/${_smtk_ui_view_CLASS_NAME}Implementation.cxx"
    @ONLY)

  set("${_smtk_ui_view_INTERFACES}"
    "${_smtk_ui_view_CLASS_NAME}Implementation"
    PARENT_SCOPE)

  set("${_smtk_ui_view_SOURCES}"
    "${CMAKE_CURRENT_BINARY_DIR}/${_smtk_ui_view_CLASS_NAME}Implementation.cxx"
    "${CMAKE_CURRENT_BINARY_DIR}/${_smtk_ui_view_CLASS_NAME}Implementation.h"
    PARENT_SCOPE)
endfunction ()
