...
- a class that implements the action of the operator, as
  a subclass of :cxx:`smtk::model::Operator`.
...
Our operator is a subclass of :cxx:`smtk::model::Operator`, which
is managed using shared pointers (it inherits :cxx:`smtkEnableSharedPtr`),
so we use the :cxx:`smtkCreateMacro()` to provide a static method for
creating a shared pointer to a new instance and
the :cxx:`smtkSharedFromThisMacro()` to override the base
class' :cxx:`shared_from_this()` method.
Finally, we invoke the :cxx:`smtkDeclareModelOperator()`.
...
+ :cxx:`ableToOperate()` which is an opportunity for an
...
+ :cxx:`operateInternal()` which implements the actual
  behavior of the operator.

Because our operator is simple, we implement :cxx:`ableToOperate()`
in the class header file. We only declare :cxx:`operateInternal()`:
...
By calling :cxx:`ensureSpecification()` in :cxx:`ableToOperate()`,
...
Operator.
Then in :cxx:`operateInternal()` we can refer to the specification
without having to verify that it is non-null; an operator's
:cxx:`operateInternal()` method will never be called unless
:cxx:`ableToOperate()` returns true.

The attribute constructed by :cxx:`ensureSpecification()` is built
...
.. include:: implement_an_operator.xml
   :start-after: <!-- +1+ -->
   :end-before: <!-- -1- -->
   :code: xml
   :number-lines:
...
We could also have made "count groups" a :cxx:`VoidItem` and tested
...
:cxx:`smtk::attribute::Attribute` instance, this time defined as
by inheriting the "result" BaseType.
The result base type includes an integer item named "outcome"
use to store one of the values in the :cxx:`OperatorOutcome` enum.
...
removed model entities in one or more :cxx:`smtk::attribute::ModelEntityItem`
instances.
...
the implementation of :cxx:`operateInternal()` can simply
fetch items from an instance of an attribute defined by the XML:

.. include:: implement_an_operator.cxx
   :start-after: // ++ 2 ++
   :end-before: // -- 2 --
   :code: c++
   :number-lines:
...
This is done using the :cxx:`smtkImplementsModelOperator()` macro:
...
+ the C++ class name (:cxx:`ex::CounterOperator`),
...
+ the SMTK component name used by the autoinitialization
  ...
  used with the :cxx:`smtkComponentInitMacro()` by adding
  this line:
